...
# Example: load last 10 posts from the "What's Science" feed and print the post text,
# data and author handle to the terminal. Does not require any authentication.
#
# It's definitely not the most efficient way to do this, but it demonstrates how to
# load single records from the API. (A more efficient way would be e.g. to connect to
# the AppView at api.bsky.app and make one call to getPosts.)
...
# to simplify, the service host is hardcoded here, ideally you should fetch the feed
# record (app.bsky.feed.generator) and read the hostname from there
FEED_HOST = "bs.bossett.io"
FEED_URI = "at://did:plc:jfhpnnst6flqway4eaeqzj2a/app.bsky.feed.generator/for-science"

# fetch the feed from the feed server (getFeedSkeleton returns only a list or URIs of
# posts); pass nil as the config file parameter to create an unauthenticated client
feed_api = Minisky.new(FEED_HOST, nil)
feed = feed_api.get_request('app.bsky.feed.getFeedSkeleton', {
  feed: FEED_URI,
  limit: 10
})
...
entries = feed['feed'].map do |r|
  ...
  print '.'
  post = bsky.get_request('com.atproto.repo.getRecord', {
    repo: did,
